...
(async () => {
  try {
    ...
    // Set up event listener for re-rendering the chart on button click
    document.querySelector("#rerenderBtn").addEventListener("click", () => {
      // Modify the data, e.g., changing a specific date's value
      const d = data.months
        .find((d) => d.month === 5)
        .days.find((d) => d.day === 7);
      ...
      if (d) {
        ...
        myTempChart.update(data); // Update the chart with new data
      }
    });
